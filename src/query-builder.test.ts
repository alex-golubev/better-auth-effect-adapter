...
describe('requireWhereClause', () => {
  ...
  it('should die for UPDATE when clause is null', async () => {
    await expect(Effect.runPromise(requireWhereClause(null, 'UPDATE'))).rejects.toThrow('UPDATE requires WHERE clause')
  })

  it('should die for DELETE when clause is null', async () => {
    await expect(Effect.runPromise(requireWhereClause(null, 'DELETE'))).rejects.toThrow('DELETE requires WHERE clause')
  })
